        ...
        if t < len(self.opt_traj):
            for param, attr in self.action_inds:
                ...
                if attr.find("grip") >= 0:
                    ...
                    val = self.opt_traj[
                        min(t, len(self.opt_traj) - 1), self.state_inds[param, attr]
                    ][0]
...
class EnvWrapper:
    def __init__(self, env, robot, mode="ee_pos", render=True):
        ...
        self.upright_rot = Rotation.from_euler("xyz", [1.57, 1.57, 0.0])
        self.upright_rot_inv = self.upright_rot.inv()
        self.flat_rot = Rotation.from_euler("xyz", [0.0, 0.0, 0.0])
        self.flat_rot_inv = self.flat_rot.inv()
        self.use_render = render
        if render:
            try:
                self.im_font = ImageFont.truetype(
                    "E:/PythonPillow/Fonts/FreeMono.ttf", 10
                )
            except:
                self.im_font = None
        else:
    ...
    def render(
        self, mode="rgb_array", resize=True, overlays=(), imsize=None, cam_ids=[-1]
    ):
        params = {
            "distance": 1.8,
            "azimuth": 90,
        ...
        if ratio > 1:
            ...
            params["crop_box"] = (
                ratio * 16.75,
                ratio * 25.0,
                params["size"] - ratio * 15.0,
                params["size"] - ratio * 31.25,
            )
        ...
        for cam_id in cam_ids:
            ...
            image = camera.render(
                depth=False, segmentation=False, overlays=mjc_overlays
            )
            camera._scene.free()  # pylint: disable=protected-access

            if resize:
                image = Image.fromarray(image).crop(box=params["crop_box"])
                image = image.resize([imsize, imsize], resample=Image.ANTIALIAS)
    ...
    def get_handle_pose(self, handle_name, order="xyzw", euler=False):
        ...
        if door.find("shelf") >= 0 or door.find("slide") >= 0:
            ...
            door_pos = np.array([0.0, 0.85, 0.0])
            pos = door_pos + const.SHELF_HANDLE_POS + np.array([val, 0.0, 0.0])

        if door.find("drawer") >= 0:
            jnt = "drawer_joint"
            val = self.env.physics.named.data.qpos[jnt][0]
            door_pos = np.array([0.0, 0.85, 0.655])
            # door_pos = self.env.physics.named.data.xpos[door]
            pos = door_pos + const.DRAWER_HANDLE_POS + np.array([0, val, 0.0])
    ...
    def get_item_pose(self, item_name, order="xyzw", euler=False):
        ...
        if item_name.find("ball") >= 0:
            ...
            if pos[2] > 0.65:
                quat = T.euler_to_quaternion([0.0, 1.0, -1.57], "wxyz")
            else:
                quat = T.euler_to_quaternion([0.0, 0.0, 0.0], "wxyz")
        ...
        if self.mode.find("joint") >= 0:
            ...
            for t in range(self.env.action_repeat):
                for _ in range(10):
                    ...
                    self.env.physics.data.qfrc_applied[
                        0:9
                    ] = self.physics.data.qfrc_bias[0:9]
                    ee_pos = self.env.physics.named.data.site_xpos["end_effector"]
                    self.env.physics.named.data.xfrc_applied["ball"][2] = 0.0
                    self.env.physics.named.data.xfrc_applied["flat_block"][2] = 0.0
                    self.env.physics.named.data.xfrc_applied["upright_block"][2] = 0.0
                    if self.env.physics.data.ctrl[-1] > 0.03:
                        if np.all(
                            np.abs(ee_pos - self.env.physics.named.data.xpos["ball"])
                            < 0.05
                        ):
                            self.env.physics.named.data.xfrc_applied["ball"][2] = -3.0
                        if np.all(
                            np.abs(
                                ee_pos - self.env.physics.named.data.xpos["flat_block"]
                            )
                            < 0.05
                        ):
                            self.env.physics.named.data.xfrc_applied["flat_block"][
                                2
                            ] = -2.0
                        ...
                        if np.all(
                            np.abs(
                                ee_pos
                                - self.env.physics.named.data.xpos["upright_block"]
                            )
                            < 0.04
                        ):
                            self.env.physics.named.data.xfrc_applied["upright_block"][
                                2
                            ] = 3.0
                    ...
                    if self.env.reward == "dense":
                        total_reward += self.env._get_task_reward(
                            self.env.task, "dense_reward"
                        )
                    elif self.env.reward == "sparse":
                        total_reward += float(
                            self.env._get_task_reward(self.env.task, "success")
                        )
                    elif self.env.reward == "success":
                        if self.env.success:
                            total_reward += (
                                0  # Only give reward once in case episode continues.
                            )
                        else:
                            self.env.success = self.env._get_task_reward(
                                self.env.task, "success"
                            )
                ...
                if (
                    ...
                    and self.env.num_steps >= self.env.episode_length
                ):
                    done = True
                else:
                    done = False
...
class RobotAgent(TAMPAgent):
    def __init__(self, hyperparams):
        ...
        self.rlen = (
            ...
            else 5 * (len(self.prob.INVARIANT_GOALS) + self.max_goals)
        )
        self._load_goals()
        self.hor = 18
        ...
        try:
            self.base_env = robodesk.RoboDesk(
                task="lift_ball",
                reward="success",
                action_repeat=freq,
                episode_length=None,
        ...
        self.mjc_env = EnvWrapper(
            self.base_env, self.panda, self.ctrl_mode, render=load_render
        )
    ...
    def get_annotated_image(self, s, t, cam_id=None):
        ...
        for goal in self.prob.GOAL_OPTIONS + self.prob.INVARIANT_GOALS:
            if not self.compound_goals:
                if targets[self.target_inds[goal, "value"]] == 1.0:
                    goalover += goal
            else:
                if targets[self.target_inds[goal, "value"]] == 1.0:
                    goalover += "1 "
        ...
        im = self.mjc_env.render(
            overlays=overlays, resize=True, imsize=112, cam_ids=self.cam_ids
        )
        # for ctxt in self.base_env.sim.render_contexts:
    ...
    def fill_sample(
        ...
        if task is not None:
            ...
            for (pname, aname), inds in self.state_inds.items():
                if aname.find("right_ee_pos") >= 0:
                    ...
                    targ_off_pose = (
                        targets[self.target_inds[targ_name, "value"]] - mp_state[inds]
                    )
                    break
            ...
            if task_name.lower() in [
                ...
                door_pos = (
                    mp_state[self.state_inds[door_name, "pose"]] + door_geom.in_pos
                )
                ...
                targ_hinge = (
                    door_geom.open_val
                    if task_name.find("open") >= 0
                    else door_geom.close_val
                )
                obj_pose = mp_state[
                    self.state_inds["{}_handle".format(door_name), "pose"]
                ]  # (targ_hinge - cur_hinge) * np.abs(door_geom.open_dir)
                sample.set(END_POSE_ENUM, obj_pose, t)
                sample.set(
                    ABS_POSE_ENUM,
                    mp_state[self.state_inds["{}_handle".format(door_name), "pose"]],
                    t,
                )
                sample.set(
                    END_ROT_ENUM, mp_state[self.state_inds[obj_name, "rotation"]], t
                ...
                targ_off_pose = mp_state[
                    self.state_inds["flat_block", "pose"]
                ]  # - obj_pose
                sample.set(END_POSE_ENUM, targ_off_pose, t)
                sample.set(
                    ABS_POSE_ENUM, mp_state[self.state_inds["flat_block", "pose"]], t
                )
                sample.set(
                    END_ROT_ENUM, mp_state[self.state_inds["flat_block", "rotation"]], t
                )
                targ_vec = np.zeros(len(prim_choices[TARG_ENUM]))
                targ_vec[:] = 1.0 / len(targ_vec)
                sample.set(TARG_ENUM, targ_vec, t)
        ...
        for i, obj in enumerate(prim_choices[OBJ_ENUM]):
            ...
            sample.set(
                OBJ_ENUMS[i], mp_state[self.state_inds[obj, "pose"]] + grasp_pt, t
            )
            sample.set(
                OBJ_DELTA_ENUMS[i],
        ...
        if fill_obs:
            if (
                ...
                if self.incl_grip_obs:
                    grip_im = (self.mjc_env.cur_obs["hand_image"] - 128.0) / 128.0
                    im = np.c_[im, grip_im]

                if self.incl_init_obs:
                    init_im = (
                        self.mjc_env.cur_obs["image"] - self.mjc_env.init_obs["image"]
                    ) / 256.0
                    im = np.c_[im, init_im]

                if self.incl_trans_obs:
                    trans_im = (
                        self.mjc_env.cur_obs["image"] - self.mjc_env.trans_obs["image"]
                    ) / 256.0
    ...
    def get_random_initial_state_vec(self, config, plans, dX, state_inds, ncond):
        ...
        for ind in order:
            ...
            key, params = self.goals[goal]
            if params[0] in used_params:
                continue
            if key in self.prob.INVARIANT_GOALS:
                continue
            i += 1
    ...
    def retime_traj(self, traj, vel=0.01, inds=None, minpts=10):
        ...
        for step in traj:
            ...
            for step in out:
                self.panda.openrave_body.set_pose(
                    step[self.state_inds["panda", "pose"]]
                )
                self.panda.openrave_body.set_dof(
                    {"right": step[self.state_inds["panda", "right"]]}
                )
                info = self.panda.openrave_body.fwd_kinematics("right")
                out[:, self.state_inds["panda", "right_ee_pos"]] = info["pos"]
                out[
                    :, self.state_inds["panda", "right_ee_rot"]
    ...
    def get_inv_cov(self):
        ...
        elif ("panda", "right_ee_pos") in self.action_inds and (
            "panda",
            "right_ee_rot",
        ) in self.action_inds:
    ...
    def clip_state(self, x):
        ...
        cv, ov = (
            self.panda.geom.get_gripper_closed_val(),
            self.panda.geom.get_gripper_open_val(),
        )
    ...
    def reward(self, x=None, targets=None, center=False):
        return 0.0  # self.base_env.reward()

    def add_viewer(self):
        if self._viewer is not None or self.cur_im is not None:
            return
    ...
    def _reload_viewer(self):
        if self._viewer is None or not self.use_glew:
            return
        ...
        self._viewer.initialize(self.physics, self._renderer, touchpad=False)
    ...
    def render_viewer(self, pixels):
        if self.use_glew and self.viewer is not None:
            with self._window._context.make_current() as ctx:
                ctx.call(
                    self._window._update_gui_on_render_thread,
                    self._window._context.window,
                    pixels,
                )
    ...
    def _in_shelf(self, x, item_name):
        ...
        return pos[0] > 0.0 and pos[1] > SHELF_Y
    ...
    def _door_open(self, x, door_name):
        ...
        sgn = -1.0 if door.geom.open_val < door.geom.close_val else 1.0
    ...
    def _stacked(self, x, item_name, base_item="flat_block"):
        ...
        return np.linalg.norm((pos1 - pos2) - [0.0, 0.0, 0.038]) < STACK_OFFSET
    ...
    def _slide(self, x, door_name, door_open=True):
        ...
        sgn = 1.0 if open_val < close_val else -1.0
        if door_open:
            return sgn * hinge_val < sgn * open_val
    ...
    def _in_drawer(self, x, obj_name):
        ...
        return (
            pos[2] < 0.7 and pos[2] > 0.5 and pos[0] > -0.3 and pos[0] < 0.3
        )  # and pos[1] > 0.35

    def set_task_info(self, sample, cur_state, t, cur_task, task_f, policies=None):
        task, policy = super(RobotAgent, self).set_task_info(
            sample, cur_state, t, cur_task, task_f, policies
        )
    ...
    def postcond_cost(self, sample, task=None, t=None, debug=False, tol=1e-3, x0=None):
        ...
        if task_name.find("place_in_door") >= 0:
            pos = x[self.state_inds[obj_name, "pose"]]
            if debug:
                print("POSTCOND INFO PLACE IN DOOR:", obj_name, door_name, pos)
            if door_name.find("shelf") >= 0:
                if pos[0] < 0.0 or pos[1] < SHELF_Y:
                    return 1.0
            elif door_name.find("drawer") >= 0:
                if not (
                    pos[0] > -0.3 and pos[0] < 0.3 and pos[2] < 0.73 and pos[2] > 0.5
                ):
                    return 1.0
            ...
            if debug:
                print(
                    ...
                    targ_name,
                    pos,
                    sample.targets[self.target_inds[targ_name, "value"]],
                )
            ...
            return 0.0 if val < -0.00453 else 1.0
