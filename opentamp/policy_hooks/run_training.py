...
USE_BASELINES = True
if USE_BASELINES:
    from opentamp.policy_hooks.baselines.argparse import (
        argsparser as baseline_argsparser,
    )
...
def get_dir_name(base, no, nt, ind, descr, args=None):
    ...
    if args is not None and not len(descr):
        ...
        curric = (
            "_curric{0}_{1}".format(args.cur_thresh, args.n_thresh)
            if args.cur_thresh > 0
            else ""
        )
        dir_name += "{0}{1}{2}{3}{4}{5}".format(
            useq, useHer, expand, curric, neg, onehot
        )
    return dir_name
...
def argsparser():
    ...
    parser.add_argument(
        "-class_label", "--classify_labels", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-obs_del", "--add_obs_delta", action="store_true", default=False
    )
    parser.add_argument(
        "-act_hist", "--add_action_hist", action="store_true", default=False
    )
    parser.add_argument(
        "-task_hist", "--add_task_hist", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-save_exp", "--save_expert", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-split_hl", "--split_hl_loss", action="store_true", default=False
    )
    parser.add_argument("-image", "--add_image", action="store_true", default=False)
    parser.add_argument(
        "-hl_image", "--add_hl_image", action="store_true", default=False
    )
    parser.add_argument(
        "-cont_image", "--add_cont_image", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-init_obs", "--incl_init_obs", action="store_true", default=False
    )
    parser.add_argument(
        "-trans_obs", "--incl_trans_obs", action="store_true", default=False
    )

    # HL args
    parser.add_argument("-check_t", "--check_prim_t", type=int, default=1)
    parser.add_argument("-n_resample", "--n_resample", type=int, default=5)
    parser.add_argument("-ff", "--ff_thresh", type=float, default=1.0)
    parser.add_argument("-hlfeed", "--hl_feedback_thresh", type=float, default=0.0)
    ...
    parser.add_argument(
        "-roll_post", "--rollout_post", action="store_true", default=False
    )
    parser.add_argument(
        "-roll_ll", "--ll_rollout_opt", action="store_true", default=False
    )
    parser.add_argument(
        "-roll_hl", "--hl_rollout_opt", action="store_true", default=False
    )
    parser.add_argument(
        "-roll_opt", "--rollout_opt", action="store_true", default=False
    )
    ...
    parser.add_argument("-prim_decay", "--prim_decay", type=float, default=1.0)
    parser.add_argument("-prim_first_wt", "--prim_first_wt", type=float, default=1e0)
    parser.add_argument("-end2end", "--end_to_end_prob", type=float, default=0.0)
    parser.add_argument("-soft", "--soft", action="store_true", default=False)
    parser.add_argument("-eta", "--eta", type=float, default=5.0)
    ...
    parser.add_argument(
        "-random", "--check_random_switch", action="store_true", default=False
    )
    parser.add_argument("-neg_ratio", "--perc_negative", type=float, default=0)
    parser.add_argument("-opt_ratio", "--perc_optimal", type=float, default=1.0)
    parser.add_argument("-dagger_ratio", "--perc_dagger", type=float, default=1.0)
    parser.add_argument("-roll_ratio", "--perc_rollout", type=float, default=0.0)
    parser.add_argument("-human_ratio", "--perc_human", type=float, default=0.0)
    ...
    parser.add_argument(
        "-neg_post", "--neg_postcond", action="store_true", default=False
    )
    ...
    parser.add_argument("-col_coeff", "--col_coeff", type=float, default=0.0)
    parser.add_argument("-expl_eta", "--explore_eta", type=float, default=5.0)
    parser.add_argument("-expl_wt", "--explore_wt", type=float, default=1.0)
    ...
    parser.add_argument(
        "-local_retime", "--local_retime", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-hl_retrain", "--hl_retrain", action="store_true", default=False
    )
    parser.add_argument(
        "-hl_only_retrain", "--hl_only_retrain", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-run_baseline", "--run_baseline", action="store_true", default=False
    )
    if USE_BASELINES:
