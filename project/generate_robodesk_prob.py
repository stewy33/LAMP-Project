...
PANDA_INIT_POSE = [0.0, 0.1, 0.55]
PANDA_INIT_ROT = [0, 0, 1.57]
PANDA_END_POSE = [0.0, 0.1, 0.55]
...
SHELF_TARGET_ROT = [1.57, 0.0, 0.0]
BIN_TARGET_POS = [0.4, 0.52, 0.75]
BIN_TARGET_ROT = [0.0, 0.0, 0.0]
OFF_DESK_TARGET_POS = [0.67, 0.54, 0.78]  # [0.68, 0.58, 0.77]
OFF_DESK_TARGET_ROT = [0.0, 0.0, 0.0]

SHELF_GEOM = "desk_shelf"
SHELF_POS = [0.0, 0.85, 0.0]
SHELF_ROT = [0.0, 0.0, 0.0]
SHELF_HANDLE_POS = (np.array(SHELF_POS) + const.SHELF_HANDLE_POS).tolist()

DESK_BODY_GEOM = [0.58, 0.27, 0.03]  # [0.6, 0.275, 0.025]
DESK_BODY_POS = [0.0, 0.85, 0.72]  # 0.735]
DESK_BODY_ROT = [0, 0, 0]

DRAWER_GEOM = "desk_drawer"
DRAWER_POS = [0.0, 0.85, 0.655]
DRAWER_ROT = [0.0, 0.0, 0.0]
DRAWER_HANDLE_POS = (np.array(DRAWER_POS) + const.DRAWER_HANDLE_POS).tolist()


def get_panda_pose_str(
    name, RArm=R_ARM_INIT, G=OPEN_GRIPPER, Pos=PANDA_INIT_POSE, Rot=PANDA_INIT_ROT
):
    s = ""
...
def write_problem(goal, filename):
    ...
    s += "Init: "
    rots = {
        "shelf_handle": const.SHELF_HANDLE_ORN,
        "upright_block": [1.57, 1.57, 0.0],
        "flat_block": [0.0, 0.0, 0.0],
    ...
    s += get_panda_pose_str(
        "robot_init_pose", R_ARM_INIT, OPEN_GRIPPER, PANDA_INIT_POSE
    )
    ...
    s += "(hinge drawer {}), ".format([0.0])
    s += "(rotation drawer {}); ".format(DRAWER_ROT)

    for item in items:
        if item.find("blue") >= 0 or item.find("red") >= 0:
            continue
...
def main():
    filename = "project/move_to_grasp.prob"
    goal = "(RobotAt panda robot_end_pose)"
    write_problem(goal, filename)
